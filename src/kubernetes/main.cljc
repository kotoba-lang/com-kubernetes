(ns kubernetes.main
  "Kotodama WASM entrypoint for the Kubernetes clean-room actor (L4) — Clojure port.
  ...
  facts (`kubernetes.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "kubernetes")
(def tier "L4")
...
(def entity-specs
  [{:entity "Pod"              :plural "pods"              :id-prefix "kubernet_pod"
    :fields [:name :namespace :phase :nodeName :podIP :qosClass :restartPolicy] :required [:name :namespace]
    :coerce {}                                              :refs {}}
   {:entity "Service"          :plural "services"          :id-prefix "kubernet_ser"
    :fields [:name :namespace :type :clusterIP :externalName]         :required [:name :namespace]
    :coerce {}                                              :refs {}}
   {:entity "Deployment"       :plural "deployments"       :id-prefix "kubernet_dep"
    :fields [:name :namespace :replicas :readyReplicas :updatedReplicas :observedGeneration] :required [:name :namespace]
    :coerce {:replicas :int :readyReplicas :int :updatedReplicas :int :observedGeneration :int} :refs {}}
   {:entity "Namespace"        :plural "namespaces"        :id-prefix "kubernet_nam"
    :fields [:name :phase :deletionTimestamp]              :required [:name :phase]
    :coerce {}                                              :refs {}}
   {:entity "Node"             :plural "nodes"             :id-prefix "kubernet_nod"
    :fields [:name :address :addressType]                  :required [:name :address]
    :coerce {}                                              :refs {}}
   {:entity "PersistentVolume" :plural "persistentvolumes" :id-prefix "kubernet_per"
    :fields [:name :phase :message]                        :required [:name :phase]
    :coerce {}                                              :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"kubernetes.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kubernetes-compat" :tier tier :entities entities} 200])
...
(defn start! [] :kubernetes-compat/ready)
